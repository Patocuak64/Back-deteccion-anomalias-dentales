...
CLASS_NAMES = {0: 'Caries', 1: 'Diente Retenido', 2: 'Pérdida Ósea'}
...
def run_inference(image: Image.Image, confidence: float) -> Tuple[Image.Image, Dict[str, Any]]:
    ...
    for cid, count in class_counts.items():
        if count > 0:
            cname = CLASS_NAMES[cid]
            report_lines.append(f"{cname}: {count} (conf prom {np.mean(class_conf[cid]):.1%})")
    report_lines += ["", "INTERPRETACIÓN:", "-"*50]
    if class_counts[0] > 0: report_lines.append(f"⚠️ Caries: {class_counts[0]}")
    if class_counts[1] > 0: report_lines.append(f"⚠️ Diente retenido: {class_counts[1]}")
    if class_counts[2] > 0: report_lines.append(f"⚠️ Pérdida ósea: {class_counts[2]}")
    if total == 0: report_lines.append("✅ Sin hallazgos significativos")
    report_lines += ["", "NOTA: Herramienta de apoyo. No reemplaza diagnóstico profesional."]
